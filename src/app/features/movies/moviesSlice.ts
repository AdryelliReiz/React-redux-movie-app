...
export const fetchAsyncMovies = createAsyncThunk(
  ...
  async (term: string) => {
    const response = await movieApi.get(
      `?apiKey=${process.env.API_KEY || "a558b683"}&s=${term}&type=movie`
    );
...
export const fetchAsyncShows = createAsyncThunk(
  ...
  async (term: string) => {
    const response = await movieApi.get(
      `?apiKey=${process.env.API_KEY || "a558b683"}&s=${term}&type=series`
    );
...
export const fetchAsyncMovieOrShowDetails = createAsyncThunk(
  ...
  async (id: string) => {
    const response = await movieApi.get(
      `?apiKey=${process.env.API_KEY || "a558b683"}&i=${id}&Plot=full`
    );
...
const movieSlice = createSlice({
  ...
  reducers: {
    ...
    removeSelectedMoviesOrShows: (state) => {
      state.selectMovieOrShow = {} as IMovieOrShowDetails;
    },
  },
...
export const { addMovies, removeSelectedMoviesOrShows } = movieSlice.actions;
...
export const getSelectMovieOrShow = (state: RootStateOrAny) =>
